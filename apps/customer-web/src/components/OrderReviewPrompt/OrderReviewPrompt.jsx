...
const OrderReviewPrompt = ({ order, onFlowChanged }) => {
  const { customerApi } = useContext(StoreContext);
  ...
  const submit = async (outcome) => {
    ...
    try {
      const response = await customerApi.post(
        `/api/order-reviews/${order._id}/${nextTarget.targetType}/${nextTarget.targetId}`,
        {
          outcome,
