    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.mapWrap}>
        <MapView style={styles.map} initialRegion={mapRegion}>
          {!routeUnavailable && route.length >= 2 ? <Polyline coordinates={route} strokeColor={colors.primary} strokeWidth={5} /> : null}
          {restaurant ? <Marker coordinate={restaurant} title={order.restaurantId?.name || "Nhà hàng"} description="Điểm lấy món" pinColor={colors.textSecondary} /> : null}
          {dropoff ? <Marker coordinate={dropoff} title="Điểm nhận hàng" description={order.shippingAddress?.address || ""} pinColor={colors.success} /> : null}
          {shipper ? <Marker coordinate={shipper} title={order.shipperId?.name || "Tài xế"} description="Đang giao hàng" pinColor={colors.primary} /> : null}
        </MapView>
      </View>

      <GlassSurface tone="soft" contentStyle={styles.statusCard}>
...
const styles = StyleSheet.create({
  ...
  mapWrap: { height: 252, overflow: "hidden", borderRadius: radius.xl, backgroundColor: colors.surfaceSubtle, borderWidth: 1, borderColor: colors.glassBorder, shadowColor: colors.primary, shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.1, shadowRadius: 28, elevation: 4 },
  map: { flex: 1 },
